...
public abstract class IGraphics {
	 ...
	 *            the ending color
	 */
	public final void drawLine(final Point a, final Color aC, final Point b,
			final Color bC) {
	 ...
	 *            the source height
	 */
	public final void drawImage(final int graphic, final int dx, final int dy,
			final int sx, final int sy, final int swidth, final int sheight) {
	 ...
	 *            the destination height
	 */
	public final void drawImage(final GraphicsObject obj, final int dx,
	 ...
	 *            the alpha mask
	 */
	public final void drawFilteredImage(final int graphic, final int dx,
			final int dy, final int sx, final int sy, final int swidth,
			final int sheight, final float opacity) {
	 ...
	 *            the alpha mask
	 */
	public final void drawFilteredImage(final GraphicsObject obj, final int dx,
			final int dy, final int dwidth, final int dheight,
			final float opacity) {
	 ...
	 * @param stretchY
	 *            streth the image on the y axis
	 */
	public final void drawTiledImage(final GraphicsObject bgImage,
	...
	/**
	 * Wraps the given text inside the given width, using the given font to
	 * create a word wrapping cache.
	 * 
	 * @param rect
	 *            the rectangle to wrap inside
	 * @param f
	 *            the font to use
	 * @param textO
	 *            the text to wrap
	 * @return a word wrapping cache with the height and split up lines
	 */
	public WordWrappingCache getWordWrappingCache(final int width,
			final Font f, final String textO) {
		...
		if (textO != null) {
			for (String text : textO.split("\n")) {
				...
				while (hasMore) {
					...
					Rectangle2D bounds = getStringBounds(f, line);
					while (bounds.getWidth() > width) {
						String[] arr = splitByWidth(line, f, width);
						result.add(arr[0]);
						line = arr[1];
		...
		return new WordWrappingCache(result, height);
	}
	 ...
	 * @param centered
	 *            if the text should be horizontally centered.
	 * @param vAlign
	 *            if the text should be vertically centered
	 */
	public final void drawWrappedText(final Rectangle rect, final Font f,
			final String textO, final Color color, final boolean centered,
			final boolean vAlign) {
		WordWrappingCache wW = getWordWrappingCache(rect.width, f, textO);
		drawWrappedText(rect, f, wW, color, centered, vAlign);
	}

	/**
	 * Draws the computed word cache in the given rectangle.
	 * 
	 * @param rect
	 ...
	 * @param centered
	 *            if the text should be horizontally aligned
	 * @param vAlign
	 *            if the text should be vertically aligned
	 */
	public final void drawWrappedText(final Rectangle rect, final Font f,
			final WordWrappingCache wW, final Color color,
			final boolean centered, final boolean vAlign) {
		int y;
		if (vAlign) {
			y = (int) (rect.getCenterY() - (wW.getHeight() / 2));
		} else {
			y = (int) rect.getY();
		}
		for (String line : wW.getLines()) {
			...
			if (y >= rect.getY()
				...
				int sX;
				if (centered) {
					sX = (int) (rect.getCenterX() - (strBounds.getWidth() / 2));
				} else {
					sX = (int) rect.getX();
				}
				if (sX >= rect.getX()
	 ...
	 * @param partAWidth
	 *            the maximum pixel width of the first part of the string.
	 * @return An array with two elements, the first being the part with the
	 *         specified pixel width, the second being the leftovers.
	 */
	protected final String[] splitByWidth(final String s, final Font f,
			final int partAWidth) {
	 ...
	 * 
	 * @param r
	 *            the new clip area, or <code>null</code> if it should be reset
	 */
