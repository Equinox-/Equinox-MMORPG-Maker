...
public class MapViewerObject extends PIContainer {
	...
	public final void setSector(final Sector sec) {
		this.sectorInfo = sec;
	}
	...
	public final void render(final IGraphics g) {
		for (TileLayer l : TileLayer.values()) {
			renderLayer(g, l);
			if (infoRender != null
					&& l == infoRender.getCurrentTileLayer()
		...
		g.drawRect((cX - xOff) * TileConstants.TILE_WIDTH,
				(cZ - zOff) * TileConstants.TILE_HEIGHT,
				TileConstants.TILE_WIDTH - 1,
				TileConstants.TILE_HEIGHT - 1);
		super.render(g);
	}

	private void renderInfoHover(final IGraphics g) {
		int[] dat = infoRender.getCurrentTiledata();
		int tileWidth = dat[3] - dat[1] + 1;
		int tileHeight = dat[4] - dat[2] + 1;
		g.drawFilteredImage(
				dat[0],
				TileConstants.TILE_WIDTH * (cX - xOff)
						+ getAbsoluteX(),
				TileConstants.TILE_HEIGHT * (cZ - zOff)
						+ getAbsoluteY(),
				dat[1] * TileConstants.TILE_WIDTH,
				dat[2] * TileConstants.TILE_HEIGHT,
				TileConstants.TILE_WIDTH
						* Math.min(tileWidth,
								SectorConstants.SECTOR_WIDTH
										- cX),
				TileConstants.TILE_HEIGHT
						* Math.min(tileHeight,
	...
	private void renderLayer(final IGraphics g, final TileLayer l) {
		int[] dat = infoRender.getCurrentTiledata();
		for (int x = Math.max(xOff, 0); x < Math.min(xOff
				+ (getWidth() / TileConstants.TILE_WIDTH),
				SectorConstants.SECTOR_WIDTH); x++) {
	...
	public final void setOffset(final int x, final int y) {
		this.xOff = x;
		this.zOff = y;
	}
	...
	private class EventHandler implements ScrollBarListener,
		...
		public void mouseDragged(final MouseEvent e) {
			...
			if (!vert.getBounds().contains(e.getX(), e.getY())
					&& !horiz.getBounds().contains(e.getX(),
							e.getY()) && infoRender != null) {
				infoRender.onMapDrag(sectorInfo, currentButton,
						cX, cZ, iCX, iCY);
			}
		}
		...
		@Override
		public void onScroll(final ScrollEvent e) {
			if (e.getSource() == vert) {
				zOff =
						(int) (e.getScrollPosition() * maxZOff) - 1;
			} else if (e.getSource() == horiz) {
				xOff =
						(int) (e.getScrollPosition() * maxXOff) - 1;
			}
		}
		...
		public void mouseClicked(final MouseEvent e) {
			if (!vert.getBounds().contains(e.getX(), e.getY())
					&& !horiz.getBounds().contains(e.getX(),
							e.getY()) && infoRender != null) {
				infoRender.onMapClick(sectorInfo, e.getButton(),
						cX, cZ, iCX, iCY);
			}
		}
