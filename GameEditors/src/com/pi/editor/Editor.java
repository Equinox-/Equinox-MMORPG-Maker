...
public class Editor implements Disposable, Renderable,
		DeviceRegistration {
	static {
		GUIKit.init();
	}
	...
	public Editor(Applet applet) {
		...
		// Create editor pages
		editorPages =
				new EditorPage[EditorState.values().length];
		editorPages[EditorState.MapEditor.ordinal()] =
				new MapRenderLoop(this);
		...
		cApplet.addComponentListener(new ComponentAdapter() {
			...
			public void componentResized(ComponentEvent e) {
				if (editState != null
					...
					editorPages[editState.ordinal()].resize(
							cApplet.getWidth(),
							cApplet.getHeight());
			}
		});
